from piecewise_regression.main import Muggeo
...
class TestMuggeo(unittest.TestCase):
    ...
    def test_with_inconsistent_start_values_and_n_breakpoints(self):

        xx = np.linspace(0, 10)
        yy = np.linspace(0, 10)

        self.assertRaises(
            ValueError, Muggeo, xx, yy,
            start_values=[3, 6], n_breakpoints=1)
